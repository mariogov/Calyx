...
aiwonder nextest gate", 2026-06-29), and **most recently refreshed through commit
`ab86c430` ("Materialize biomedical evidence substrate in Aster graph", 2026-07-03)** for
interface and invariant changes across the 188 commits since `dd500623`. New surfaces
tracked in the latest (`ab86c430`) refresh:
...
`ab86c430` the workspace holds **23 crates** — two were added since `dd500623`:
`calyx-buildinfo` (embedded build identity, #1108) and `calyx-fsv` (canonical
`CALYX_FSV_ROOT` resolution, #1014). The remaining pinned invariants are **unchanged** and
re-verified at `ab86c430`: MCP tools **31** (`tests/stdio.rs::EXPECTED_TOOLS`), column
families **33** (`ColumnFamily::STATIC`), core error codes **38** (`CALYX_ERROR_CODES` /
PRD 18), binaries **5** (`calyx`, `calyxd`, `calyx-mcp`, `calyx-web-api`,
`calyx-hazard-soak`), toolchain **1.95.0** / edition **2024**, version **0.1.0**. None of
the refreshes was a line-by-line re-audit of every subsystem doc. Every statement is derived from code; design intent from
the repo's own planning docs is summarized in doc 23 and clearly distinguished from
implementation.
...
| | |
|---|---|
| Crates | 23 (Rust workspace, edition 2024, toolchain 1.95.0) — adds `calyx-buildinfo` (#1108) and `calyx-fsv` (#1014) since `dd500623`; `calyx-search` and `calyx-web-api` were added earlier, since the 2e7b9a2 baseline |
...
holds **23 crates** — `calyx-buildinfo` (issue #1108, embedded build identity) and
`calyx-fsv` (issue #1014, canonical FSV evidence-root resolution) were added since
`dd500623`, and `calyx-search` (issue #573/#900, the shared search path) and
`calyx-web-api` (issue #889, the HTTP surface) were added earlier, since the 2e7b9a2 baseline.
...
| Crates | 23 (adds `calyx-buildinfo`, `calyx-fsv`, `calyx-search`, `calyx-web-api`) |
...
> **Baseline note (refresh through `96cbc84f`, 2026-06-27).** The detailed per-crate file
...
> workspace has grown to **23 crates / ~1,460 in-`src/` `.rs` files** (`git ls-files
> 'crates/*/src/**/*.rs'` at `ab86c430`): four crates were added — `calyx-search` (issue
...
> evidence-root resolution) — and existing crates gained source (notably the media-ingest,
> web-API wiring, resident GPU worker, ONNX Runtime, biomedical-substrate, and
> kernel/blind-spot CLI work). Treat the tables below as the `2e7b9a2` map; the newer crates
> and their interfaces are documented in docs 01, 17, 20, and 25.
...
|---|---|---|
| Crates (workspace members) | **23** (↻ ab86c430; was 21 at `dd500623` — adds `calyx-buildinfo` #1108, `calyx-fsv` #1014; earlier adds `calyx-search`, `calyx-web-api`) | `ls crates/` |
| Binaries | **5** (↻ 96cbc84f; `calyx`, `calyxd`, `calyx-mcp`, `calyx-web-api`, `calyx-hazard-soak`) | `grep -l '[[bin]]' crates/*/Cargo.toml` |
| Tracked `.rs` files (total) | **1,256** (2e7b9a2 baseline) | `git ls-files '*.rs'` |
| `.rs` files under `crates/*/src/` | **~1,460** (↻ ab86c430; ~1,164 at 96cbc84f; was 877 at 2e7b9a2) | `git ls-files 'crates/*/src/**/*.rs'` |
...
| CLI structured subcommands | **~23** + legacy + domain command groups (↻ adds `rebuild-search-index`, `kernel-build`, `blind-spot-sweep`) | [20](20_cli_and_daemon_reference.md) |
