...
pub(in crate::persisted::multi) struct BinarySegmentSearchSpec<'a> {
    pub(in crate::persisted::multi) path: &'a Path,
    pub(in crate::persisted::multi) sha256: &'a str,
    pub(in crate::persisted::multi) row_count: u64,
    pub(in crate::persisted::multi) token_count: u64,
}
...
pub(in crate::persisted::multi) fn score_binary_segment(
    ...
    let mut reader = BufReader::new(file);
    let mut hasher = Sha256::new();
    let header = read_binary_header_hashed(spec.path, &mut reader, &mut hasher)?;
    validate_binary_segment_header(
        &header,
        slot,
        expected_token_dim,
    ...
    for _ in 0..header.row_count {
        ...
        let row_token_count = read_u32(spec.path, &mut reader, &mut hasher)? as u64;
        observed_tokens = observed_tokens
            .checked_add(row_token_count)
            .ok_or_else(|| stale("persistent binary multi sidecar token_count overflow"))?;
    ...
    ensure_no_trailing_bytes(spec.path, &mut reader, &mut hasher)?;
    let actual = finish_sha256_hex(hasher);
